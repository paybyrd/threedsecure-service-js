export default class ThreeDSecureService {
	...
	preAuthV2({ id }, correlationId) {
		return this._retry(
			...
			return this._executeDsMethod(preAuthResponse).then(() => ({
				id,
				...preAuthResponse,
			}));
		});
	...
	_executeDsMethod(preAuthResponse) {
		return new Promise((resolve, reject) => {
			try {
				...
				threeDSMethodDataInput.value =
					this._convertToBase64UriJson(threeDSMethodData);
				form.submit();
	...
	_executeChallengeV1(authResponse) {
		return new Promise((resolve, reject) => {
			try {
				...
				const paReqInput = this._createInput(form, paReqInputName, inputType);

				const termUrlInput = this._createInput(form, termUrlInputName, inputType);
	...
	_executeChallengeV2(authResponse) {
		return new Promise((resolve, reject) => {
			try {
				...
				this._createInput(form, threeDSSessionDataInputName, inputType);
	...
	_getBrowserData() {
		...
		const colorDepth = allowedBrowserColorDepth.find(
			(x) => x <= screen.colorDepth
		);
		return {
